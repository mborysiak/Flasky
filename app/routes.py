...
def index():
    ...
    # create the submit post form and add it to the database
    form = PostForm()
    ...
    # find the page number of posts or default to 1 and show pagination of posts followed by current user
    page = request.args.get('page', 1, type=int)
    posts = current_user.followed_posts().paginate(page, app.config['POSTS_PER_PAGE'], False)

    # set up next and previous page urls if there are more pages to be displayed
...
def explore():
    ...
    # show all posts ordered by timestamp with pagination
    page = request.args.get('page', 1, type=int)
    ...
    # return the index template, but without the form argument to prevent post form from showing
...
def user(username):
    ...
    # set up the pagination and pull all posts from the user
    page = request.args.get('page', 1, type=int)
